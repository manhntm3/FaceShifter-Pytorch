...
class IdentityLoss(nn.Module):
    def __init__(self):
        ...
        self.l1 = nn.L1Loss()
    def forward(self, z_id_X, z_id_Y):
        inner_product = torch.relu((torch.bmm(z_id_X.unsqueeze(1), z_id_Y.unsqueeze(2)).squeeze()))
        return self.l1(torch.ones_like(inner_product), inner_product)

class IdentityContrastiveLoss(nn.Module):
    def __init__(self):
        super(IdentityContrastiveLoss, self).__init__()
        self.l1 = nn.L1Loss()
    def forward(self, z_id_s, z_id_t, z_id_f, same):
        loss = 0
        inner_product = torch.relu((torch.bmm(z_id_s.unsqueeze(1), z_id_f.unsqueeze(2)).squeeze()))
        similar = self.l1(torch.ones_like(inner_product), inner_product)

        inner_product = torch.relu((torch.bmm(z_id_t.unsqueeze(1), z_id_f.unsqueeze(2)).squeeze()))
        same = same.squeeze(-1)
        same = same.expand(inner_product.shape)
        inner_product = torch.mul(inner_product, 1-same)
        contrast = self.l1(torch.zeros_like(inner_product), inner_product)

        return similar + 0.3 * contrast

class AttributeLoss(nn.Module):
    def __init__(self):
        super(AttributeLoss, self).__init__()
        self.l2 = nn.MSELoss()
    def forward(self, z_att_X, z_att_Y):
        loss = 0
        for i in range(8):
